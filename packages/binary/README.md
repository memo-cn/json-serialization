...
-   Considering that `Buffer` is not supported in a web environment, `Blob` and `File` may not be supported in a Node.js environment. During deserialization, `@json-serialization/binary` may convert these three data types to and from each other to ensure adaptation to the running environment.

-   In order to prevent `Buffer` from being preprocessed by `JSON.stringify`, `@json-serialization/binary` will internally set `Buffer.prototype.toJSON` to `null`, please pay attention to and handle the possible impact.
