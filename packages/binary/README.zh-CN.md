...
-   考虑到在 Web 环境中，`Buffer` 不受支持，在 Node.js 环境中可能也不支持 `Blob` 和 `File`。在反序列化时，`@json-serialization/binary` 可能会对这 3 种数据类型进行相互转换，以确保适应运行环境。

-   为了防止 `Buffer` 被 `JSON.stringify` 预处理，`@json-serialization/binary` 内部会将 `Buffer.prototype.toJSON` 设置为 `null`，请注意并处理可能的影响。
